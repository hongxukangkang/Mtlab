...
h = prewitt_fspecial('prewitt');  % 应用prewitt算子锐化图像
R1 = filter2(h,R);          % prewitt算子滤波锐化
G1 = filter2(h,G);          % prewitt算子滤波锐化
B1 = filter2(h,B);          % prewitt算子滤波锐化
im1 = cat(3,R1,G1,B1);      % prewitt算子滤波锐化图像
figure('color',[1,1,1])
subplot(121),imshow(im,[]),title('original image')
subplot(122),imshow(im1,[]),title('prewitt锐化滤波器')